...
func gasSStore(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	var (
		...
		val  = evm.StateDB.GetState(contract.Address(), common.BigToHash(x), uint32(evm.BlockNumber.Uint64()-1))
	)
...
func gasCall(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if eip158 {
		if transfersValue && evm.StateDB.Empty(address, readBlockNr) {
			gas += params.CallNewAccountGas
		}
	} else if !evm.StateDB.Exist(address, readBlockNr) {
		gas += params.CallNewAccountGas
	}
	if transfersValue {
...
func gasSuicide(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if evm.ChainConfig().IsEIP150(evm.BlockNumber) {
		...
		if eip158 {
			...
			if evm.StateDB.Empty(address, readBlockNr) && evm.StateDB.GetBalance(contract.Address(), readBlockNr).Sign() != 0 {
	...
	if !evm.StateDB.HasSuicided(contract.Address(), readBlockNr) {
