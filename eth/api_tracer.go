		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					task.statedb.Finalise(true, task.tds.TrieStateWriter())
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			if time.Since(logged) > 8*time.Second {
				if number > origin {
					log.Info("Tracing chain segment", "start", origin, "end", end.NumberU64(), "current", number, "transactions", traced, "elapsed", time.Since(begin), "memory", api.eth.ChainDb().Size())
				} else {
			...
			err = statedb.Commit(api.eth.chainConfig.IsEIP158(big.NewInt(int64(number))), tds.DbStateWriter())
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	*/
	...
	for i, tx := range block.Transactions() {
		...
		// Finalize the state so any modifications are written to the trie
		statedb.Finalise(true, state.NewNoopWriter())
