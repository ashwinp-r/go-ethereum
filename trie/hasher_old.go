...
var hasherPoolOld = sync.Pool{
	New: func() interface{} {
		return &hasherOld{tmp: new(bytes.Buffer), sha: sha3.NewKeccak256()}
	},
}

func newHasherOld(encodeToBytes bool) *hasherOld {
	h := hasherPoolOld.Get().(*hasherOld)
	h.encodeToBytes = encodeToBytes
	return h
}

func returnHasherToPoolOld(h *hasherOld) {
	hasherPoolOld.Put(h)
}

// hash collapses a node down into a hash node, also returning a copy of the
// original node initialized with the computed hash to replace the original one.
func (h *hasherOld) hash(n node, force bool) (node, error) {
	// If we're not storing the node, just hashing, use available cached data
	if !n.dirty() {
		return hashNode(n.hash()), nil
	}
...
func (h *hasherOld) hashChildren(original node) (node, error) {
	var err error
...
func (h *hasherOld) store(n node, force bool) (node, error) {
