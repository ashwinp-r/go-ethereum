...
type request struct {
	...
	bucket []byte    // Database bucket distinguishing between the tries
...
type syncMemBatch struct {
	buckets map[common.Hash][]byte // Corresponding buckets
...
func newSyncMemBatch() *syncMemBatch {
	return &syncMemBatch{
		...
		buckets: make(map[common.Hash][]byte),
...
func NewTrieSync(bucket []byte, root common.Hash, database DatabaseReader, callback LeafCallback) *TrieSync {
	ts := &TrieSync{
		database: database,
	...
	ts.AddSubTrie(bucket, root, 0, common.Hash{}, callback)
	return ts
}

// AddSubTrie registers a new trie to the sync code, rooted at the designated parent.
func (s *TrieSync) AddSubTrie(bucket []byte, root common.Hash, depth int, parent common.Hash, callback LeafCallback) {
	...
	key := root.Bytes()
	blob, _ := s.database.Get(bucket, key)
...
// contract code).
func (s *TrieSync) AddRawEntry(bucket []byte, hash common.Hash, depth int, parent common.Hash) {
