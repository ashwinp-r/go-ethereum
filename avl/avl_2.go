...
func (t *Avl2) walkToArrowPoint(r Ref2, key []byte, height uint32) (point Ref2, parent *Fork2, parentC int) {
	current := r
	for {
		...
		case *Leaf2:
			if height != 1 || !bytes.Equal(key, n.key) {
				panic(fmt.Sprintf("Leaf2 with key %s, expected height %d key %s", n.key, height, key))
			}
			return n, nil, 0
		case *Fork2:
			if n.height < height {
...
func (t *Avl2) scan(r Ref2, m map[PageID]struct{}) int {
	...
	case *Arrow2:
		...
		point, _, _ := t.walkToArrowPoint(root, r.max, r.height)
...
func (t *Avl2) delete(current Ref2, key []byte) Ref2 {
	...
	case *Fork2:
		...
		switch nl := t.Peel(n.left).(type) {
		case *Leaf2:
			switch nr := t.Peel(n.right).(type) {
			case *Leaf2:
				if trace {
